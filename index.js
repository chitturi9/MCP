...
app.post("/sms", async (req, res) => {
  ...
  try {
    const msg = await telnyx.messages.create({ from: FROM_NUMBER, to, text });
    res.json({ success: true, id: msg.data.id, status: msg.data.to[0].status });
  } catch (e) {
...
// ── Webhook receiver (Telnyx events) ─────────────────────────
app.post("/webhook", (req, res) => {
  const event = req.body?.data;
  if (event) {
    console.log(`📩 Webhook event: ${event.event_type}`, JSON.stringify(event.payload, null, 2));
  }
  res.sendStatus(200);
});
